...
# try:
#     # for older TF-Addons versions
#     from tensorflow_addons.utils.keras_utils import LossFunctionWrapper
# except ImportError:
#     # use the built-in version in tf.keras.losses
...
def load_batch(ids, img_index, tags_index,
    ...
    for i in ids:
        if 'train' in img_index[i]:
            # images_path = '../train/'
            images_path = '/home/annachatz/imageclef2025/images/'
        else:
            # images_path = '../valid/'
            images_path = '/home/annachatz/imageclef2025/images/'
        ...
        if hasattr(preprocessor, 'preprocess_input'):
            ...
            ## the below for efficientNet
            # x = np.squeeze(preprocessor(img))
            ## the below for densenet121
            img_array = np.array(img).astype('float32')
            x = np.squeeze(preprocessor(img_array))
        x_data.append(x)
...
def load_test_batch(ids, img_index, images_path, preprocessor, size=(224, 224, 3)):
    ...
    for i in ids:
        ...
        if 'test' in img_index[i]:
           images_path = '/media/SSD_2TB/imageclef2025/test/'
        else:
            images_path = '/home/annachatz/imageclef2025/images/' 
